...
###박씨네 집 클래스
class HousePark:
	lastname = "박" #lastname이라는 필드를 초기화 해준다.
...
#pey = HousePark()
#pey.travel("부산")
#이렇게 하면 오류가 난다. pey.setname("응용")이부분이 있어야 이름이 완성되고 travel이 실행 가능해진다.
...
##상속
#박씨네집을 이용해서 김씨네집을 만들어 보자.
class HouseKim(HousePark): #이렇게 해서 HousePark을 상속받는다.
	lastname = "김" #수정되는 부분만 적어준다.
#이것과 완전 동일하게 작용한다.
#class HouseKim:
#      lastname = "김"
#    def __init__(self, name):
